...
from common.platform import forwarded_headers, install_platform

app = FastAPI(title="Baker Street Checkout", version="1.1.0")
tracer = install_platform(app, "checkout")
...
async def ready():
    ...
    async with httpx.AsyncClient(timeout=1.0) as client:
        for name, url in {"catalog": CATALOG_URL, "payments": PAYMENTS_URL, "orders": ORDERS_URL}.items():
            try:
                checks[name] = (await client.get(f"{url}/health")).status_code == 200
            except Exception:
                checks[name] = False
...
async def do_checkout(body: CheckoutIn):
    ...
    current.set_attribute("baker_street.sku", body.sku)
    current.set_attribute("baker_street.quantity", body.quantity)
    if FAULT["delay_ms"]:
        current.add_event("fault injection delay", {"fault.name": FAULT["name"], "fault.delay_ms": FAULT["delay_ms"]})
        with tracer.start_as_current_span("injected checkout latency") as span:
            span.set_attribute("fault.name", FAULT["name"] or "none")
            span.set_attribute("fault.delay_ms", FAULT["delay_ms"])
            await asyncio.sleep(FAULT["delay_ms"] / 1000)
    headers = forwarded_headers()
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        product = await client.get(f"{CATALOG_URL}/products/{body.sku}", headers=headers)
        ...
        payment = await client.post(
            ...
            headers=headers,
            json={"amount_cents": total, "token": body.payment_token},
        )
        ...
        order = await client.post(
            ...
            json={
                ...
                "total_cents": total,
                "authorization_id": pay["authorization_id"],
            },
        )
        order.raise_for_status()
        current.set_attribute("baker_street.order_id", order.json()["id"])
        return order.json()
...
async def checkout_post(body: CheckoutIn):
    ...
    try:
        result = await do_checkout(body)
        REQS.labels("/checkout", "200").inc()
        return result
    except HTTPException:
        REQS.labels("/checkout", "4xx").inc()
        raise
    except Exception as exc:
        REQS.labels("/checkout", "500").inc()
        raise HTTPException(502, str(exc))
    finally:
        LAT.labels("/checkout").observe(time.perf_counter() - start)
