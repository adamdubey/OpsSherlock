...
app=FastAPI(title="Baker Street Catalog",version="1.0.0")
install_platform(app,"catalog")
...
@app.get("/products")
async def products(): return {"products":list(PRODUCTS.values())}

@app.get("/items")
async def legacy_items(): return {"items":[{"sku":p["sku"],"stock":p["stock"]} for p in PRODUCTS.values()]}
...
async def product(sku:str):
    ...
    if not item: raise HTTPException(404,"unknown sku")
